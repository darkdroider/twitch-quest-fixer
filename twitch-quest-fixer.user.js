...
(function() {
    ...
    if (!monitoredChannels.includes(channel)) {
        console.log('Channel not monitored:', channel);
        return;
    }

    const checkApolloClient = () => {
        if (typeof __APOLLO_CLIENT__ === 'undefined') {
            console.log('Apollo client not found, retrying...');
            setTimeout(checkApolloClient, 1000);
            return;
        }

        try {
            const channelData = __APOLLO_CLIENT__.cache.data.data.ROOT_QUERY["channel({\"name\":\""+channel+"\"})"];
            if (!channelData) {
                console.log('Channel data not found for:', channel);
                return;
            }

            const channelId = channelData.__ref.split(":")[1];
            ...
            const grantPermission = async () => {
                ...
                if (newAuthToken) {
                    ...
                    console.log("Permission granted successfully. You should see another message saying that you're earning ARP in about 60 seconds.");
                };
            }

            const handlePolling = () => {
                console.log('Polling for ARP tracking...');
            ...
            console.log('Error initializing Twitch Quest Fixer:', error);
        }
    };
